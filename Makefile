...
$(BUILD_DIR)/texteditor.js: $(wildcard ./src/texteditor/*.ts) $(wildcard ./lib/*.ts)
	@mkdir -p $(BUILD_DIR)
	@$(TYPESCRIPT) --target ES5 --sourceMap --outfile $@ $^

$(BUILD_DIR)/texteditor.min.js: $(BUILD_DIR)/texteditor.js
	@$(UGLIFY) $^ --source-map -o $@

$(BUILD_DIR)/javascript.js: $(wildcard ./src/javascript/*.ts)
	@mkdir -p $(BUILD_DIR)
	@$(TYPESCRIPT) --target ES5 --sourceMap --outfile $@ $^
...
$(BUILD_DIR)/html.js: $(wildcard ./src/html/*.ts)
	@mkdir -p $(BUILD_DIR)
	@$(TYPESCRIPT) --target ES5 --sourceMap --outfile $@ $^
...
$(BUILD_DIR)/css.js: $(wildcard ./src/css/*.ts)
	@mkdir -p $(BUILD_DIR)
	@$(TYPESCRIPT) --target ES5 --sourceMap --outfile $@ $^
